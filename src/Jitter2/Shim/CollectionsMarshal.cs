...
namespace System.Runtime.InteropServices
{
    ...
    internal static class CollectionsMarshal
    {
        ...
        public static void SetCount<T>(List<T> list, int count)
        {
            ...
            if (ListSize == 3)
            {
                ref var listData = ref Unsafe.As<List<T>, ListViewA<T>>(ref list);
                ref int version = ref listData._version;
                version++;
                ref T[] items = ref listData._items;
                ref int size = ref listData._size;

                if (count > list.Capacity) list.Grow(count);
                else if (count < size && RuntimeHelpers.IsReferenceOrContainsReferences<T>())
            ...
            {
                ref var listData = ref Unsafe.As<List<T>, ListViewB<T>>(ref list);
                ref int version = ref listData._version;
                version++;
                ref T[] items = ref listData._items;
                ref int size = ref listData._size;
    ...
    internal static class ListExtensions
    {
        ...
        private static int GetNewCapacity<T>(this List<T> list, int capacity)
        {
            ...
            if (ListSize == 3)
            {
                var listData = Unsafe.As<List<T>, ListViewA<T>>(ref list);
                _items = listData._items;
            }
            else
            {
                var listData = Unsafe.As<List<T>, ListViewB<T>>(ref list);
                _items = listData._items;
            }
